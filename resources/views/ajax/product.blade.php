              ...
              <table id="example1" class="table table-bordered table-striped animated fadeIn">
               ...
               @forelse($products as $product)
                ...
                <tr id="item-{{$product->id}}">
                  <td>{{$product->id}}</td>
                  <td>{{$product['name']}}</td>
                  <td> @if(!empty($pictures)) <img src="images/product/{{$firstPicture}}" style="width: 50px;height: 50px;" alt="23">@else{!! '<span class="btn btn-warning">Chưa có ảnh</span>'!!} @endif</td>
